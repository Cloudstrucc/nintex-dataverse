...
<a id="elections-canada-power-platform-monitoring-architecture-and-implementation-build-book-ibm-qradar-variant"></a>
# Elections Canada Power Platform Monitoring Architecture and Implementation Build Book - IBM QRadar Variant
...
- [Elections Canada Power Platform Monitoring Architecture and Implementation Build Book - IBM QRadar Variant](#elections-canada-power-platform-monitoring-architecture-and-implementation-build-book-ibm-qradar-variant)
  ...
  - [7. Detailed monitoring design](#7-detailed-monitoring-design)
    ...
    - [7.5 QRadar-specific SIEM routing requirement](#7-5-qradar-specific-siem-routing-requirement)
  ...
  - [Appendix B - IBM QRadar Security Monitoring Build Book](#appendix-b-ibm-qradar-security-monitoring-build-book)
    - [B.1 Purpose](#b-1-purpose)
    - [B.2 Prerequisites](#b-2-prerequisites)
    - [B.3 QRadar artifact names](#b-3-qradar-artifact-names)
    - [B.4 Configure Microsoft 365 / Purview audit ingestion](#b-4-configure-microsoft-365-purview-audit-ingestion)
    - [B.5 Configure Entra External ID ingestion through Azure Event Hubs](#b-5-configure-entra-external-id-ingestion-through-azure-event-hubs)
    - [B.6 Create QRadar custom event properties](#b-6-create-qradar-custom-event-properties)
    - [B.7 Create reference sets](#b-7-create-reference-sets)
    - [B.8 Create QRadar rules and offenses](#b-8-create-qradar-rules-and-offenses)
    - [B.9 QRadar validation and handoff](#b-9-qradar-validation-and-handoff)
  - [Appendix C - Entra External ID Monitoring Build Book](#appendix-c-entra-external-id-monitoring-build-book)
    ...
    - [C.3 Create Event Hubs for QRadar ingestion](#c-3-create-event-hubs-for-qradar-ingestion)
    - [C.4 Create storage account for QRadar Event Hubs checkpointing](#c-4-create-storage-account-for-qradar-event-hubs-checkpointing)
    - [C.5 Route Entra External ID logs to Event Hubs](#c-5-route-entra-external-id-logs-to-event-hubs)
    - [C.6 Configure QRadar Microsoft Entra ID log sources](#c-6-configure-qradar-microsoft-entra-id-log-sources)
    - [C.7 Required External ID QRadar rules](#c-7-required-external-id-qradar-rules)
...
| Primary implementation teams | IT Operations for Azure Monitor; IT Security/SOC for IBM QRadar; Identity team for Entra External ID; Compliance team for Microsoft Purview |
...
- Elections Canada will use **IBM QRadar** as the SIEM for Power Platform security monitoring. This includes Power Platform administrative activity, DLP policy changes, custom connector changes, destructive environment actions, Dataverse/audit investigation signals, Entra External ID sign-in risk, external registration failure spikes, and application credential or consent changes.
- Elections Canada will use **Microsoft Purview** as the audit, compliance, DLP evidence, retention, and eDiscovery support layer. Purview will not replace Azure Monitor or IBM QRadar; it will provide authoritative audit evidence and compliance investigation records.
...
<div class="diagram" role="img" aria-label="Architecture diagram for Power Platform monitoring with Azure Monitor, Purview and IBM QRadar">
<svg viewBox="0 0 1080 500" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="665" y="118" font-size="18" font-weight="700" fill="#6a0032">IBM QRadar</text>
  <text x="665" y="148" font-size="14" fill="#333">Correlation rules</text><text x="665" y="170" font-size="14" fill="#333">Offenses</text><text x="665" y="192" font-size="14" fill="#333">AQL searches</text><text x="665" y="214" font-size="14" fill="#333">SOC workflows</text>
...
![Target monitoring architecture - IBM QRadar SIEM variant](qradar_architecture_flow.svg)
...
- IBM QRadar will generate security detections and SOC-owned incidents/offenses when activity is unauthorized, suspicious, risky, malicious, or policy-relevant.
...
- The build steps for this architecture are implemented in **Appendix A**, **Appendix B - IBM QRadar Security Monitoring Build Book**, **Appendix C**, and **Appendix D**.
...
Elections Canada will implement IBM QRadar to provide the security monitoring and incident response layer for Power Platform and Entra External ID. IBM QRadar will:
...
| IT Security / SOC | Implement IBM QRadar rules, incidents/offenses, hunting searches, and security notification workflows. | Investigate security alerts, maintain SIEM detection content, tune false positives through change control. |
...
<a id="7-5-qradar-specific-siem-routing-requirement"></a>
### 7.5 QRadar-specific SIEM routing requirement

Elections Canada will route Entra External ID diagnostic logs to Azure Event Hubs and configure QRadar to collect those events with the Microsoft Entra ID DSM and Microsoft Azure Event Hubs protocol. Elections Canada will route Microsoft 365 / Microsoft Purview audit events to QRadar using the Microsoft Office 365 DSM and Office 365 REST API protocol. This approach provides QRadar visibility into Power Platform audit activity exposed through Purview and Entra External ID sign-in/audit activity exposed through Azure Event Hubs.
...
**QRadar AQL / rule search expression**

```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Office 365%' AND (UTF8(payload) ILIKE '%Audit%' AND UTF8(payload) ILIKE '%Disable%') LAST 15 MINUTES
```
...
```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Office 365%' AND (UTF8(payload) ILIKE '%DeleteEnvironment%' OR UTF8(payload) ILIKE '%ResetEnvironment%') LAST 15 MINUTES
```
...
```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Office 365%' AND UTF8(payload) ILIKE '%DlpPolicy%' LAST 30 MINUTES
```
...
```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Office 365%' AND UTF8(payload) ILIKE '%Connector%' LAST 30 MINUTES
```
...
```sql
SELECT username, sourceip, QIDNAME(qid) AS EventName, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Entra%' AND (UTF8(payload) ILIKE '%Power Pages%' OR UTF8(payload) ILIKE '%PowerApps Portals%') AND (UTF8(payload) ILIKE '%risk%' OR UTF8(payload) ILIKE '%failure%') LAST 15 MINUTES
```
...
```sql
SELECT COUNT(*) AS Failures, UNIQUECOUNT(sourceip) AS DistinctIPs FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Entra%' AND UTF8(payload) ILIKE '%Power Pages%' AND UTF8(payload) ILIKE '%failure%' LAST 30 MINUTES
```
...
```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime FROM events WHERE LOGSOURCENAME(logsourceid) ILIKE '%Entra%' AND (UTF8(payload) ILIKE '%service principal%' OR UTF8(payload) ILIKE '%application%' OR UTF8(payload) ILIKE '%consent%') LAST 30 MINUTES
```
...
<a id="appendix-qradar"></a>
<a id="appendix-b-ibm-qradar-security-monitoring-build-book"></a>
## Appendix B - IBM QRadar Security Monitoring Build Book

<a id="b-1-purpose"></a>
### B.1 Purpose

Elections Canada will implement IBM QRadar as the SIEM layer for Power Platform and Entra External ID security monitoring where this variant is selected. QRadar will receive Microsoft 365/Purview audit data for Power Platform activity, Entra External ID logs through Azure Event Hubs, and relevant Microsoft cloud security events. QRadar will generate offenses, support AQL searches, and provide SOC correlation rules.
...
- QRadar Console administrative access is required.
- QRadar must have the current Microsoft Office 365 DSM and Office 365 REST API protocol packages installed.
- QRadar must have the current Microsoft Entra ID DSM and Microsoft Azure Event Hubs protocol packages installed.
- Network egress from QRadar to Microsoft 365 and Azure endpoints is required.
- Network connectivity to Azure Event Hubs over the IBM-documented ports is required.
- A Microsoft Entra app registration is required for the Office 365 Management Activity API integration.
- A certificate or secret must be stored according to Elections Canada secret-management standards.

<a id="b-3-qradar-artifact-names"></a>
### B.3 QRadar artifact names

| QRadar artifact | Recommended name | Purpose |
|---|---|---|
| Log source - Purview/M365 audit | `EC-M365-Purview-PowerPlatform-Audit` | Pulls Microsoft 365/Purview audit records, including Power Platform activity where available. |
| Log source - Entra sign-ins | `EC-EntraExternalID-SignIn` | Ingests External ID sign-in events from Azure Event Hubs. |
| Log source - Entra audit | `EC-EntraExternalID-Audit` | Ingests External ID audit/directory events from Azure Event Hubs. |
| Reference set - production environments | `REF-EC-PP-PROD-ENVIRONMENTS` | Stores production environment IDs/names for rule scoping. |
| Reference set - Power Pages apps | `REF-EC-EXTID-POWERPAGES-APPS` | Stores application IDs/display names used for external portal sign-in. |
| Rule group | `EC Power Platform Security Monitoring` | Contains Power Platform SIEM rules. |
| Rule group | `EC Entra External ID Monitoring` | Contains External ID SIEM rules. |

<a id="b-4-configure-microsoft-365-purview-audit-ingestion"></a>
### B.4 Configure Microsoft 365 / Purview audit ingestion

#### Step B.4.1 - Create the Entra app registration for QRadar Office 365 REST API
...
- Set **Name** to `APP-EC-QRADAR-O365-AUDIT-PROD`.
- Set **Supported account types** to **Accounts in this organizational directory only**.
- Select **Register**.
- Record **Application (client) ID** and **Directory (tenant) ID**.
- Select **Certificates & secrets**.
- Upload the approved certificate or create a client secret only if certificate authentication is not available.
- Select **API permissions**.
- Select **Add a permission**.
- Select **Office 365 Management APIs**.
- Select **Application permissions**.
- Add `ActivityFeed.Read`.
- Add `ActivityFeed.ReadDlp` if DLP events are required.
- Add service health permissions only if service communications are required and supported by the current QRadar protocol.
- Select **Grant admin consent for Elections Canada**.
- Record the app registration details in the secure implementation record.

**Azure PowerShell / Microsoft Graph alternative**

```powershell
Connect-MgGraph -TenantId '<EC-TENANT-ID>' -Scopes 'Application.ReadWrite.All','AppRoleAssignment.ReadWrite.All','Directory.Read.All'
$app = New-MgApplication -DisplayName 'APP-EC-QRADAR-O365-AUDIT-PROD' -SignInAudience 'AzureADMyOrg'
$sp = New-MgServicePrincipal -AppId $app.AppId
# Upload certificate credentials and assign Office 365 Management API application permissions according to the approved permission grant procedure.
# Complete admin consent through the portal or approved Graph automation.
```

#### Step B.4.2 - Enable and validate Microsoft 365 unified audit logging

- Complete Appendix D before connecting QRadar.
- Validate that Power Platform audit records are visible in Microsoft Purview.
- Validate that the Office 365 Management Activity API can access audit content after the app registration is authorized.

#### Step B.4.3 - Add the QRadar Microsoft Office 365 log source

**QRadar console configuration**

- Sign in to the QRadar Console.
- Open **Admin**.
- Open **Log Source Management**.
- Select **New Log Source**.
- Set **Log Source Type** to **Microsoft Office 365**.
- Set **Protocol Configuration** to **Office 365 REST API**.
- Set **Log Source Identifier** to `EC-M365-Purview-PowerPlatform-Audit`.
- Set **Tenant ID** to `<EC-TENANT-ID>`.
- Set **Client ID** to `<APP-EC-QRADAR-O365-AUDIT-PROD-CLIENT-ID>`.
- Configure certificate/private key or client secret according to the QRadar protocol form.
- Set **Management Activity API URL** to `https://manage.office.com` unless Elections Canada requires a sovereign endpoint.
- Set **Azure AD Sign-in URL** to `https://login.microsoftonline.com` unless a different Microsoft cloud endpoint is required.
- Select audit content types required for collection, including Azure Active Directory audit, SharePoint audit, Exchange audit, General audit, and DLP if approved.
- Set polling interval according to QRadar EPS and Microsoft API throttling expectations.
- Save the log source.
- Select **Test** if the QRadar Log Source Management app supports test connectivity for the protocol.
- Deploy changes.

#### Step B.4.4 - Validate Power Platform audit events in QRadar

**QRadar validation**

- Open **Log Activity**.
- Filter by log source `EC-M365-Purview-PowerPlatform-Audit`.
- Search the implementation window.
- Validate that events parse as Microsoft Office 365 records.
- Search for Power Platform terms such as `PowerPlatform`, `PowerApps`, `Environment`, `DlpPolicy`, `Connector`, and `Dataverse`.
- Save the search as `SEARCH-EC-PP-AUDIT-RECENT-ACTIVITY`.

**AQL validation**

```sql
SELECT starttime, username, sourceip, QIDNAME(qid) AS EventName, UTF8(payload) AS RawEvent
FROM events
WHERE LOGSOURCENAME(logsourceid) = 'EC-M365-Purview-PowerPlatform-Audit'
LAST 24 HOURS
```

<a id="b-5-configure-entra-external-id-ingestion-through-azure-event-hubs"></a>
### B.5 Configure Entra External ID ingestion through Azure Event Hubs

- Complete Appendix C to route Entra External ID sign-in and audit logs to Event Hubs.
- In QRadar, add log source `EC-EntraExternalID-SignIn` using **Log Source Type: Microsoft Entra ID** and **Protocol Configuration: Microsoft Azure Event Hubs**.
- In QRadar, add log source `EC-EntraExternalID-Audit` using the same DSM/protocol and the audit Event Hub.
- Deploy changes and validate events in **Log Activity**.

<a id="b-6-create-qradar-custom-event-properties"></a>
### B.6 Create QRadar custom event properties

**QRadar console configuration**

- Open **Admin**.
- Open **Custom Event Properties**.
- Create or validate these custom event properties after sample events are ingested:
  - `EC_AppDisplayName`
  - `EC_EnvironmentId`
  - `EC_EnvironmentName`
  - `EC_PowerPlatformOperation`
  - `EC_UserPrincipalName`
  - `EC_RiskLevel`
  - `EC_ResultType`
  - `EC_CorrelationId`
  - `EC_ConditionalAccessStatus`
- Use the QRadar property extraction workflow against sample raw events.
- Test each property against at least 10 sample events.
- Save the custom properties.
- Re-run the saved searches and confirm the properties populate.

<a id="b-7-create-reference-sets"></a>
### B.7 Create reference sets

**QRadar console configuration**

- Open **Admin**.
- Open **Reference Set Management**.
- Create `REF-EC-PP-PROD-ENVIRONMENTS` as an alphanumeric reference set.
- Add every production Power Platform environment ID and environment name that must be monitored.
- Create `REF-EC-EXTID-POWERPAGES-APPS` as an alphanumeric reference set.
- Add every Entra application ID and display name associated with Power Pages sign-in and registration.
- Assign maintenance ownership to Platform Engineering.

<a id="b-8-create-qradar-rules-and-offenses"></a>
### B.8 Create QRadar rules and offenses

**QRadar console configuration pattern**

- Open **Offenses**.
- Select **Rules**.
- Select **Actions**.
- Select **New Event Rule**.
- Set the rule name to the exact alert name from section 9.2.
- Place the rule in the group `EC Power Platform Security Monitoring` or `EC Entra External ID Monitoring`.
- Set the rule test to match the required log source and the relevant operation, result, risk, or application property.
- Set threshold and time window according to section 9.2.
- Configure rule response to **Create Offense**.
- Set offense indexing by actor, source IP, application, or environment depending on the alert.
- Add SOC notification response if Elections Canada QRadar notification integration is enabled.
- Save the rule.
- Deploy changes.
- Repeat for every security alert in section 9.2.

**Example rule logic - DLP policy changed**

```sql
SELECT QIDNAME(qid) AS EventName, username, sourceip, starttime, UTF8(payload) AS RawEvent
FROM events
WHERE LOGSOURCENAME(logsourceid) = 'EC-M365-Purview-PowerPlatform-Audit'
  AND UTF8(payload) ILIKE '%DlpPolicy%'
LAST 30 MINUTES
```

**Example rule logic - External ID failure spike**

```sql
SELECT COUNT(*) AS FailureCount, UNIQUECOUNT(sourceip) AS SourceIPCount
FROM events
WHERE LOGSOURCENAME(logsourceid) = 'EC-EntraExternalID-SignIn'
  AND (UTF8(payload) ILIKE '%failure%' OR UTF8(payload) ILIKE '%ResultType%')
LAST 30 MINUTES
```

<a id="b-9-qradar-validation-and-handoff"></a>
### B.9 QRadar validation and handoff

- Confirm events arrive from each QRadar log source.
- Confirm Microsoft Office 365 audit records include Power Platform activity from Purview.
- Confirm Entra External ID sign-in and audit events are received from Event Hubs.
- Confirm custom event properties parse consistently.
- Trigger a controlled non-production event and confirm the QRadar rule creates an offense.
- Attach QRadar log source screenshots, sample parsed events, rule screenshots, and offense evidence to the RFC.
...
Elections Canada will monitor Entra External ID because external-facing Power Pages sites use Entra External ID for external user sign-in and registration. The implementation will route Entra External ID activity to Azure Monitor for operational query and to IBM QRadar for SIEM correlation.
...
<a id="c-3-create-event-hubs-for-qradar-ingestion"></a>
### C.3 Create Event Hubs for QRadar ingestion
...
- Set **Subscription** to `SUB-EC-PLATFORM-PROD`.
- Set **Resource group** to `RG-EC-PP-MON-PROD-CAC`.
- Set **Namespace name** to `EHNS-EC-SIEM-PROD-CAC`.
- Set **Location** to `Canada Central`.
- Set **Pricing tier** according to Elections Canada EPS/throughput estimates. Start with Standard unless the QRadar architecture requires Premium/Dedicated.
- Select **Review + create**.
- Select **Create**.
- Open the namespace.
- Select **Event Hubs**.
- Select **+ Event Hub**.
- Create `EH-EC-ENTRA-SIGNIN-PROD`.
- Create `EH-EC-ENTRA-AUDIT-PROD`.
- Open **Shared access policies**.
- Create or validate a policy named `QRadarListen` with **Listen** permission only.
- Copy the connection string for QRadar configuration and store it according to Elections Canada secret-handling requirements.
...
New-AzEventHubNamespace `
  -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' `
  -NamespaceName 'EHNS-EC-SIEM-PROD-CAC' `
  -Location 'canadacentral' `
  -SkuName Standard

New-AzEventHub -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' -NamespaceName 'EHNS-EC-SIEM-PROD-CAC' -Name 'EH-EC-ENTRA-SIGNIN-PROD' -PartitionCount 4 -MessageRetentionInDays 7
New-AzEventHub -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' -NamespaceName 'EHNS-EC-SIEM-PROD-CAC' -Name 'EH-EC-ENTRA-AUDIT-PROD' -PartitionCount 4 -MessageRetentionInDays 7
New-AzEventHubAuthorizationRule -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' -NamespaceName 'EHNS-EC-SIEM-PROD-CAC' -Name 'QRadarListen' -Rights @('Listen')
```

<a id="c-4-create-storage-account-for-qradar-event-hubs-checkpointing"></a>
### C.4 Create storage account for QRadar Event Hubs checkpointing

**Portal configuration**

- In the Azure portal, search for **Storage accounts**.
- Select **Create**.
- Set **Name** to `stecsiemcheckpointprod`.
- Set **Region** to `Canada Central`.
- Set **Performance** to **Standard**.
- Set **Redundancy** to the approved Elections Canada standard.
- Select **Review + create** and then **Create**.
- Open the storage account.
- Create a container named `qradar-checkpoints`.
- Create a shared access policy or access key according to the QRadar protocol requirements and Elections Canada secret management standard.
...
New-AzStorageAccount `
  -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' `
  -Name 'stecsiemcheckpointprod' `
  -Location 'canadacentral' `
  -SkuName 'Standard_GRS'
$ctx = (Get-AzStorageAccount -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' -Name 'stecsiemcheckpointprod').Context
New-AzStorageContainer -Name 'qradar-checkpoints' -Context $ctx -Permission Off
```

<a id="c-5-route-entra-external-id-logs-to-event-hubs"></a>
### C.5 Route Entra External ID logs to Event Hubs

**Portal configuration**

- Sign in to the Microsoft Entra admin center.
- Switch to the External ID tenant if it is separate from the workforce tenant.
- Browse to **Entra ID**.
- Select **Monitoring & health**.
- Select **Diagnostic settings**.
- Select **Add diagnostic setting**.
- Set **Diagnostic setting name** to `DIAG-EC-EXTID-TO-EH-PROD`.
- Select the required log categories in section C.2.
- Select **Stream to an event hub**.
- Select subscription `SUB-EC-PLATFORM-PROD`.
- Select Event Hubs namespace `EHNS-EC-SIEM-PROD-CAC`.
- Select event hub `EH-EC-ENTRA-SIGNIN-PROD` for sign-in categories and `EH-EC-ENTRA-AUDIT-PROD` for audit categories. If the portal permits only one Event Hub per diagnostic setting, create two diagnostic settings: one for sign-in categories and one for audit categories.
- Select policy `QRadarListen` only for QRadar consumption where applicable.
- Select **Save**.

**Azure PowerShell alternative**

```powershell
$ehRuleId = (Get-AzEventHubAuthorizationRule -ResourceGroupName 'RG-EC-PP-MON-PROD-CAC' -NamespaceName 'EHNS-EC-SIEM-PROD-CAC' -Name 'QRadarListen').Id
$logs = @('SignInLogs','NonInteractiveUserSignInLogs','ServicePrincipalSignInLogs') | ForEach-Object { New-AzDiagnosticSettingLogSettingsObject -Category $_ -Enabled $true }
New-AzDiagnosticSetting `
  -Name 'DIAG-EC-EXTID-SIGNIN-TO-EH-PROD' `
  -ResourceId '/providers/microsoft.aadiam' `
  -EventHubAuthorizationRuleId $ehRuleId `
  -EventHubName 'EH-EC-ENTRA-SIGNIN-PROD' `
  -Log $logs
```

<a id="c-6-configure-qradar-microsoft-entra-id-log-sources"></a>
### C.6 Configure QRadar Microsoft Entra ID log sources

**QRadar console configuration**

- Sign in to the QRadar Console with an account authorized to manage log sources.
- Open **Admin**.
- Open **Log Source Management**.
- Select **New Log Source**.
- Set **Log Source Type** to **Microsoft Entra ID**.
- Set **Protocol Configuration** to **Microsoft Azure Event Hubs**.
- Set **Log Source Identifier** to `EC-EntraExternalID-SignIn` for sign-in logs.
- Set **Event Hub Connection String** using the `QRadarListen` listener connection string.
- Set **Event Hub Name** to `EH-EC-ENTRA-SIGNIN-PROD`.
- Set **Consumer Group** to `$Default` unless Elections Canada creates a QRadar-specific consumer group such as `qradar`.
- Set **Storage Account Connection String** to the checkpoint storage account connection string.
- Set **Container Name** to `qradar-checkpoints`.
- Save the log source.
- Repeat the same configuration for `EC-EntraExternalID-Audit` using `EH-EC-ENTRA-AUDIT-PROD`.
- Deploy changes.
- Validate that events are arriving in **Log Activity**.

<a id="c-7-required-external-id-qradar-rules"></a>
### C.7 Required External ID QRadar rules

- Implement `EC-EXTID-SEC-P1-External-User-Risky-Sign-In` from section 9 as a QRadar correlation rule that creates an offense.
- Implement `EC-EXTID-SEC-P2-Registration-Failure-Spike` from section 9 as a QRadar threshold rule by log source and application.
- Implement `EC-EXTID-SEC-P2-App-Credential-Or-Consent-Changed` from section 9 as a QRadar correlation rule against Entra audit events.
- Create or validate custom event properties for `AppDisplayName`, `ResultType`, `IPAddress`, `RiskLevel`, `CorrelationId`, `UserPrincipalName`, and `ConditionalAccessStatus` after sample event ingestion.
...
- Validate that IBM QRadar receives the required security events.
...
- [IBM QRadar DSM for Microsoft Office 365](https://www.ibm.com/docs/en/SS42VS_DSM/com.ibm.dsm.doc/c_dsm_guide_microsoft_office_365_overview.html)
- [IBM QRadar Office 365 REST API protocol configuration options](https://www.ibm.com/docs/en/dsm?topic=options-office-365-rest-api-protocol-configuration)
- [IBM QRadar Microsoft Office 365 account configuration in Microsoft Azure Active Directory](https://www.ibm.com/docs/en/dsm?topic=mo3-configuring-microsoft-office-365-account-in-microsoft-azure-active-directory)
- [IBM QRadar DSM for Microsoft Entra ID](https://www.ibm.com/docs/en/dsm?topic=microsoft-entra-id)
- [IBM QRadar Microsoft Entra ID log source parameters](https://www.ibm.com/docs/en/dsm?topic=id-microsoft-entra-log-source-parameters)
- [IBM QRadar Microsoft Azure Event Hubs protocol setup](https://www.ibm.com/docs/en/dsm?topic=options-configuring-microsoft-azure-event-hubs-communicate-qradar)
- [IBM QRadar Microsoft Azure Event Hubs protocol configuration options](https://www.ibm.com/docs/SS42VS_DSM/com.ibm.dsm.doc/c_logsource_Microsoft_Azure_Event_Hubs_protocol.html)
- [IBM QRadar supported DSMs](https://www.ibm.com/docs/en/dsm?topic=configuration-qradar-supported-dsms)