...
show_help() {
  ...
  OPTIONS:
    --config, -c <file>    Path to environment config file (.env-deploy)
    --help, -h             Show this help message

  MODES (prompted at runtime):
    1) Deploy specific records by GUID(s)
       - Enter a table name (e.g. powerpagecomponents)
       - Enter comma-separated GUIDs to deploy

    2) Deploy all records from table(s)
       - Enter comma-separated table names
       - Optionally filter by website ID
  ...
  CONFIG FILE FORMAT (.env-deploy):
    SRC_TENANT_ID="..."
    SRC_CLIENT_ID="..."
    SRC_CLIENT_SECRET="..."
    SRC_ENVIRONMENT_URL="https://source.crm3.dynamics.com"
    TGT_TENANT_ID="..."
    TGT_CLIENT_ID="..."
    TGT_CLIENT_SECRET="..."
    TGT_ENVIRONMENT_URL="https://target.crm3.dynamics.com"

  SUPPORTED TABLES:
    powerpagecomponents    Web templates, site settings, content snippets
    mspp_webtemplates      Legacy web templates (standard data model)
    mspp_contentsnippets   Legacy content snippets
    cs_templates           Signature templates
    cs_envelopes           Signature envelopes
    annotations            Note attachments
    (any Dataverse table)  Auto-detects primary key

  DEPLOYMENT PLAN:
    Before deploying, the tool shows a table of all records with:
    - GUID, Name, Last Modified, Action (Create or Update)
    You must confirm (y/n) before any changes are made.

  NOTES:
    - Only the target environment is modified; source is read-only
    - Site settings are NOT touched unless explicitly included
    - OData metadata fields are stripped before PATCH/POST
    - Records are upserted: updated if they exist, created if not
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo -e "${RED}Unknown argument: $1. Use --help for usage.${NC}"; exit 1 ;;
  esac
done
